defmodule Burnin.Worker.QueueSimple do
  @moduledoc """
  Queue Simple pattern burn-in worker.
  Uses multiple sender clients for high-throughput concurrent sends,
  and receives via the simple (unary) API.
  """
  ...
  def init(opts) do
    ...
    senders_list =
      for i <- 1..@sender_pool_size do
        case KubeMQ.Client.start_link(address: broker, client_id: "burnin-queue-simple-send-#{i}") do
          {:ok, pid} -> pid
          {:error, _} -> nil
        end
      end
    ...
    {:ok, receiver} = KubeMQ.Client.start_link(address: broker, client_id: "burnin-queue-simple-recv")
  ...
  defp start_loops(receiver, state) do
    ...
    recv_tasks =
      Enum.map(state.channels, fn ch ->
        Task.async(fn -> receive_loop(receiver, ch, state.queue_config, worker) end)
      end)
  ...
  defp receive_loop(client, channel, queue_cfg, worker) do
    max_msgs = Map.get(queue_cfg, :poll_max_messages, 10)
    wait_ms = Map.get(queue_cfg, :poll_wait_timeout_seconds, 5) * 1_000

    case KubeMQ.Client.receive_queue_messages(client, channel, max_messages: max_msgs, wait_timeout: wait_ms) do
      {:ok, result} ->
        count = length(Map.get(result, :messages, []))
        if count > 0, do: send(worker, {:recv_count, channel, count})

      {:error, _} ->
        send(worker, {:recv_error, channel})
        Process.sleep(1_000)
    end

    receive_loop(client, channel, queue_cfg, worker)
  end
  ...
  defp generate_channels(count) do
    Enum.map(1..count, &"burnin-queue-simple-#{&1}")
  end
end
