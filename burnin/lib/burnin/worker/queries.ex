defmodule Burnin.Worker.Queries do
  @moduledoc """
  Queries pattern burn-in worker.
  Subscribes as a responder and sends queries at a configured rate.
  ...
  def init(opts) do
    ...
    senders_list =
      for i <- 1..@sender_pool_size do
        case KubeMQ.Client.start_link(address: broker, client_id: "burnin-queries-send-#{i}") do
          {:ok, pid} -> pid
          {:error, _} -> nil
        end
      end
    ...
    if state.running && state.sender_count > 0 do
      ...
      for i <- 0..(batch - 1) do
        ...
        spawn(fn ->
          ...
          case KubeMQ.Client.send_query(sender, query) do
            {:ok, %{executed: true}} ->
              send(worker, {:query_ok, channel})

            _ ->
              send(worker, {:query_error, channel})
          end
        end)
      end
  ...
  def handle_info({:query_ok, channel}, state) do
  ...
  def handle_info({:query_error, channel}, state) do
  ...
  defp responder_loop(responder, channel) do
    ...
    case KubeMQ.Client.subscribe_to_queries(responder, channel, on_query: on_query, group: "burnin") do
      {:ok, _sub} -> Process.sleep(:infinity)
      {:error, reason} -> Logger.debug("Queries subscribe error: #{inspect(reason)}")
    end
  end
  ...
  defp cleanup_channels(state) do
    ...
    if client && Process.alive?(client) do
      Enum.each(state.channels, fn ch ->
        try do
          KubeMQ.Client.delete_channel(client, ch, :queries)
        catch
          _, _ -> :ok
        end
      end)
  ...
  defp generate_channels(count) do
    Enum.map(1..count, &"burnin-queries-#{&1}")
  end
end
