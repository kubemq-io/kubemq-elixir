defmodule KubeMQ.Events.Subscriber do
  @moduledoc false

  use GenServer
  require Logger

  alias KubeMQ.{Error, EventReceive}
  ...
  def handle_info({:stream_event, proto_event}, state) do
    event = EventReceive.from_proto(proto_event)
    dispatch_event(state, event)
    {:noreply, state}
  end

  def handle_info({:stream_error, reason}, state) do
    Logger.warning("[Events.Subscriber] Stream error on #{state.channel}: #{inspect(reason)}")
  ...
  def handle_info({:stream_closed, _reason}, state) do
    Logger.info("[Events.Subscriber] Stream closed on #{state.channel}")
  ...
  def handle_info({:EXIT, pid, reason}, state) when pid == state.recv_pid do
    Logger.warning("[Events.Subscriber] recv_loop exited: #{inspect(reason)}")
    {:noreply, %{state | recv_pid: nil}, {:continue, :resubscribe}}
  end
      ...
      is_function(state.on_event, 1) ->
        Task.Supervisor.start_child(state.task_supervisor, fn ->
          try do
            ...
            e ->
              Logger.error("[Events.Subscriber] on_event callback error: #{Exception.message(e)}")
          end
        end)

      is_pid(state.notify) ->
        Kernel.send(state.notify, {:kubemq_event, event})
    ...
    if is_function(state.on_error, 1) do
      Task.Supervisor.start_child(state.task_supervisor, fn ->
        try do
          ...
          e ->
            Logger.error("[Events.Subscriber] on_error callback error: #{Exception.message(e)}")
        end
      end)
    end
  end
